...
def quaternion_to_transform_matrix(translation, quaternion):  
    """  
    将位置和四元数转换为齐次变换矩阵。  
  
    参数:  
    translation : numpy.ndarray  
        形状为(3,)的数组，表示平移（x, y, z）。  
    quaternion : numpy.ndarray  
        形状为(4,)的数组，表示四元数（w, x, y, z）。  
  
    返回:  
    numpy.ndarray  
        形状为(4, 4)的齐次变换矩阵。  
    """  
    # 创建Rotation对象，从四元数初始化  
    ...
    transform_matrix[:3, :3] = rot_matrix  # 将旋转矩阵填充到前3x3部分  
    transform_matrix[:3, 3] = translation  # 将平移向量填充到最后一列  
...
class KittiEvalOdom():
    ...
    def eval(self, gt_dir, result_dir, source_dir=None, alignment=None, seqs=None):
        ...
        if source_dir is not None:
            available_seqs = sorted(glob(os.path.join(source_dir, "*.txt")))
            sequences = [int(i[-6:-4]) for i in available_seqs if i[-6:-4] in seq_list]
            for i in sequences:
                src_file_name = source_dir + "/" + '{:02}'.format(i) + ".txt"
                res_file_name = result_dir + "/" + '{:02}'.format(i) + ".txt"
                print(src_file_name, res_file_name)
                fout = open(res_file_name, mode = 'w')
                count = [0, 0, 0, 0, 0]
                # pre_data = [0 for i in range(7)]
                with open(src_file_name, mode='r') as fin:
                    for line in fin:
                        l = line.strip()
                        if l == "poses:":
                            count[0]+=1
                ...
                with open(src_file_name, mode='r')as fin:
                    ...
                    while 1:
                        line = fin.readline().strip()
                        if count[3] >= count[2] or line == 'poses:':
                            break
                        count[3] += 1
                        dat = []
                        if line == 'position:':
                        ...
                        if len(dat) == 7:
                            count[4] += 1
                            translation = np.array(dat[0:3]) # - np.array(pre_data[0:3])
                            quaternion = np.array(dat[3:]) # - np.array(pre_data[3:])
                            transform_matrix = quaternion_to_transform_matrix(translation, quaternion)  
                            fout.write(str(count[4]) + ' ')
                            for i in range(3):
                                for j in range(4):
                                    fout.write(str(transform_matrix[i][j]) + ' ')
                            fout.write("\n")
        ...
        for i in self.eval_seqs:
            ...
            print(len(poses_gt), len(poses_result))
